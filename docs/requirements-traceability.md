...
| ----------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CRON-F01    | Met    | GitHub route/YAML filtering, metadata-only blob paths, and unified/split PR line recovery in `githubCronAdapter.ts`; saved DOM fixtures and adapter tests               |
...
| CRON-F03    | Met    | Adjacent 20px namespaced button injection in a copy-safe sibling slot without replacing source text in `cronLens.tsx`; runtime and Chromium E2E tests                   |
...
| CRON-F05    | Met    | GitHub timezone/default UTC, five-field/operator/value/macro/interval/top-of-hour rules; tests include standard names/ranges and unsupported extensions                 |
| CRON-F06    | Met    | Kubernetes `spec.timeZone`, controller-dependent unknown time, official macros, standard field syntax, and embedded `TZ` / `CRON_TZ` rejection with tests               |
