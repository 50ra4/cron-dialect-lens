...
| CRON-F05    | Met    | GitHub timezone/default UTC, documented field ranges/operators, macro/interval/top-of-hour rules; tests include unsupported extensions and comments                      |
| CRON-F06    | Met    | Kubernetes `spec.timeZone`, controller-dependent time, macros, `?`, documented field ranges, and embedded `TZ` / `CRON_TZ` rejection with tests                          |
