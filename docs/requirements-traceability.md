...
| --- | --- | --- |
| CRON-F01 | Met | GitHub route/YAML filtering and blob/PR line recovery in `githubCronAdapter.ts`; three saved DOM fixtures and adapter tests |
| CRON-F02 | Met | Path, key, YAML-document context, confidence, and `unknown-posix` fallback in `detectDialect.ts`; unit and incomplete-diff fixture tests |
